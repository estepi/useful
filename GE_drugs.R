...
#filter
cpms <- cpm(y, prior.count = 0.01, log = F)
head(cpms)
...
forPlot <-
    cbind(t1$logFC.groupSUDKB, t1$PValue, t2$logFC[ii], t2$PValue[ii])
head(forPlot)
...
getwd()
all <-
    read.table("cRPKM_AND_COUNTS-Hsa37.tab",
               header = T,
               row.names = 1)
cn <- colnames(all)
...
dim(subset)#19847
nonzero <- as.matrix(subset[rowSums(subset) > 0,])
dim(nonzero)#16834
y <- DGEList(counts = nonzero, group = group)
plotMDS(y)
####divide en IP vs ALL
#group 1: IP
ip <- subset[, grep("IP", colnames(subset))]
dim(ip)
groupip <- c(
    "DMSO_IP",
    "DMSO_IP",
...
#########################################################
##group 2: all
total <- subset[,-grep("IP", colnames(subset))]
dim(ip)
head(total)
grouptotal <- c("DMSO", "DMSO",
                ...
                "SUDK", "SUDK",
                "SUDC1", "SUDC1")
nonzero <- as.matrix(total[rowSums(total) > 0,])
dim(nonzero)#16686
class(nonzero)
y <- DGEList(counts = nonzero, group = grouptotal)
plotMDS(y)
...
###add names:
tablaIP <- read.table("all_vs_dmso_IP.txt",
                      header = T,
                      row.names = 1)
head(tablaIP)
head(all)
...
write.table(tablaIPfinal,
            file = "all_vs_dmso_IP.tab",
            quote = F,
            sep = "\t")
#################################
tablaTotal <-
    read.table("all_vs_dmso_total.txt",
               header = T,
               row.names = 1)
head(tablaTotal)
head(all)
...
write.table(tablaTotalfinal,
            file = "all_vs_dmso_total.tab",
            quote = F,
            sep = "\t")
