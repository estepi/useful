...
library(edgeR)
setwd("")
table<-read.table("geneCounts_319.tab", header=T, row.names = 1) ;dim(table)#52465
##############
...
#############################################################
#nonzero:
nonzero<-table[rowSums(table[,-1])>0,-1];dim(nonzero)#44275
length(condition)
y <- DGEList(counts=nonzero, group=condition[-1])
#filter: 100% smaples more than 1cpm 
keep <- rowSums(cpm(y)>1)==319; length(which(keep))#6947
write.table(rownames(y$counts), file="filteredGenes_1cpm_100.txt", quote = F, sep="\t")
...
###################################################
write.table(lrt$table, file="lrt_table_hg19_HF.tab", sep="\t", col.names=NA)
dim(y)
