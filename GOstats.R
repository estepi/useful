...
head(notNa)
write.table(vtNames[is.na(ii)], file="GeneNames_without_ENSIds.txt", quote=F)
#queryes for GO
...
df7<-cbind(df6, vtName=unique$GENE[bb], equal=df6$hgnc_symbol==unique$GENE[bb])
getwd()
write.table(df7, file="/home/emancini/Dropbox (CRG)/Personal_Gosia/Shared/GosiaAndEstefi/ensembl/Ensembl_GO_hgnc_vtools.txt", quote=F)
##############################################################
...
names(entrez)
length(entrez)
universeEntrezI<-match(as.character(universeGeneIds), names(entrez)); 
length(universeEntrezI); length(which(is.na(universeEntrezI)))#240
universe<-entrez[universeEntrezI]; length(universe)
uc<-universe[!is.na(universe)]; length(uc)#5633
###########################################################################
...
sc<-selected[!is.na(selected)]; length(sc)#539
...
write.table(df, "/home/emancini/Dropbox (CRG)/Personal_Gosia/Shared/GosiaAndEstefi/GO/AC008073.5_GO.txt", sep="\t")
head(df)
######################################################
...
selectedEntrezI<-match(as.character(sf3b1Selected), names(entrez));  
...
head(df2)
write.table(df2, "/home/emancini/Dropbox (CRG)/Personal_Gosia/Shared/GosiaAndEstefi/GO/SF3B1_GO_over_classical.txt", sep="\t")
#######################################################
params<-new("GOHyperGParams",
            geneIds=sc,
            universeGeneIds=uc,
...
write.table(df3, "/home/emancini/Dropbox (CRG)/Personal_Gosia/Shared/GosiaAndEstefi/GO/SF3B1_GO_under_classical.txt", sep="\t")
dim(df3)
head(df2)
head(df4)
#######################################################
...
write.table(df4, "/home/emancini/Dropbox (CRG)/Personal_Gosia/Shared/GosiaAndEstefi/GO/SF3B1_GO_over_conditional.txt", sep="\t")
dim(df4)
