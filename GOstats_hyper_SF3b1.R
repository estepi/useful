...
library(dplyr)
library(ggplot2)
dPSI<-read.csv("ensembl/forGO/df1_dPSI15t.txt", sep="\t", header=T)
##############################################################
vtNames <- as.character(dPSI[, 1])
ensembleIds <-
  read.csv(
    "ensembl/inputFiles/fullConversionTable_vtools_ens.csv",
    sep = ",",
    header = T
  )
...
##############################
testDF <-
  data.frame(CWC22 = testDF[, colnames(testDF) == "CWC22"], SF3B1 = testDF[, colnames(testDF) ==
                                                                             "SF3B1"])
colnames(testDF)
...
dfPlot[1:5, ]
dfSpreadEF <- spread(dfPlot, KD, value = EF, fill = 1)
head(dfSpreadEF)

dfPlot2 <- data.frame(unlist(rn), pvalsul, KD = nn)
head(dfPlot2)
colnames(dfPlot2) <- c("GO", "Pval", "KD")
dfSpreadPv <-
  spread(dfPlot2, KD, value = Pval, fill = 1)
head(dfSpreadPv)

dfPlot3 <- data.frame(unlist(rn), sizetot, KD = nn)
head(dfPlot3)
colnames(dfPlot3) <- c("GO", "Size", "KD")
dfSpreadSize <- spread(dfPlot3, KD, value = Size, fill = 1)

dfPlot4 <- data.frame(unlist(rn), countsTot, KD = nn)
head(dfPlot4)
colnames(dfPlot4) <- c("GO", "Count", "KD")
dfSpreadCount <- spread(dfPlot4, KD, value = Count, fill = 1)

dfPlot5 <- data.frame(unlist(rn), terms, KD = nn)
head(dfPlot5)
...
dfFinal <-
  data.frame(dfSpreadEF, dfSpreadPv, dfSpreadCount, dfSpreadSize)
