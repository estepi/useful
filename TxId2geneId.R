...
#######################################################
#benjamin predicion
ben3<-read.table("ben/output-ensg-3-sigma-input-estefi-ensgs.txt", header=T);head(ben3)
ben4<-read.table("ben/output-ensg-4-sigma-input-estefi-ensgs.txt", header=T);head(ben4)
ben5<-read.table("ben/output-ensg-5-sigma-input-estefi-ensgs.txt", header=T);head(ben5)
ben6<-read.table("ben/output-ensg-6-sigma-input-estefi-ensgs.txt", header=T);head(ben6)
#########################################################
ENSGID3<- getBM(attributes = c("ensembl_gene_id", "ensembl_transcript_id"),
        filters    = "ensembl_transcript_id",
        values     = unique(ben3$transcript_enst), 
        mart       = ensembl)
#conversion table
dfBenInt<-data.frame(source=ben3$rbp_ensg, target=ben3$transcript_enst); dim(dfBenInt); head(dfBenInt)
ii<-match(dfBenInt$target, ENSGID3$ensembl_transcript_id)
dfBenInt$targetENS<- ENSGID3$ensembl_gene_id[ii];head(dfBenInt)
############################################################
...
target2vast<-match(as.character(dfBenInt$targetENS),
                   as.character(conversion$FGeneID)); length(which(is.na(target2vast)))#7873
