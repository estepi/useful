dPSI <- read.csv("forGO/df1_dPSI15t.txt", sep = "\t", header = T)
...
genesLRT <-
  read.table("lrt_table_hg19_HF.tab",
             header = T,
             row.names = 1)
genesLRT <- read.table("lrt_table_hg19.tab",
                       header = T,
                       row.names = 1)
kdsNames <- sub("logFC.group", "", colnames(genesLRT))

kdsNames <- kdsNames[-c(306:308)]
length(kdsNames)
vectorLFC <- vector(length = length(kdsNames))
names(vectorLFC) <- kdsNames
dd <- match(names(vectorLFC), genesCounts$X)
length(which(is.na(dd)))
df <-
  data.frame(FC = vectorLFC,
             symbol = genesCounts$X[dd],
             ens = row.names(genesCounts)[dd])
head(df)
length(which(is.na(match(
  df$ens, rownames(genesLRT)
))))#59 #19
##############################################################################
...
sI <- match(df$symbol, colnames(genesLRT))
length(sI)
gI <- match(df$ens, rownames(genesLRT))
length(gI)
names(gI) <- df$symbol
names(sI) <- df$symbol
df[1:4, 1:4]
