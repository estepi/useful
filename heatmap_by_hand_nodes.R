...
dim(kdPPI)#466
#################################################################
int <- read.table("../interactions/edgelist_intN",
                  sep = "\t",
                  header = T)
colnames(int) <- c("number", "source", "target")
head(int)
#################################################################
iclip <-
  read.table("../interactions/edgelist_iclip",
             sep = "\t",
             header = T)
...
#ppi
n3 <- unique(c(
  as.character(kdPPI$geneVastO),
  as.character(kdPPI$geneVastD)
))
length(n3)#367
#iclip
n4 <- unique(c(as.character(iclip$source),
               as.character(iclip$target)))
length(n4)#57
#exones all
n5 <- unique(c(
...
head(forPlotMelt)
#################################################################
pdf("presence_ausence_nodes.pdf",
    width = 8.27,
    height = 11.69)
...
length(edgesR)#389
edgesPPI <-
  paste(kdPPI$geneVastO, kdPPI$geneVastD, sep = "->")
length(edgesPPI)#466
edgesICLIP <-
  paste(iclip$source, iclip$target, sep = "->")
length(edgesICLIP)#82
...
head(forPlot)
table(forPlot$edges)
forPlot <- forPlot[rowSums(forPlot[, 1:6]) >= 2, ]
dim(forPlot)
#119
forPlot[forPlot == TRUE] <- 1
forPlot[forPlot == FALSE] <- 0
...
head(forPlotMelt)
#################################################################
pdf("presence_ausence_edges.pdf",
    width = 8.27,
    height = 11.69)
