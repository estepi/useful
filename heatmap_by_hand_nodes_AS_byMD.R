...
head(centLC)
#################################################################
edgeListLR<-read.table("rnaseq_edgelist.tab");head(edgeListLR); dim(edgeListLR)#583
...
head(alt5all)
cent5AltAll<-read.table("Alt5_op_CentralityRanking.tab", sep="\t", header=T, row.names = 1);
##################################################################
#IR
IRall<-read.table("IR_op_edgelist.tab", sep="\t", header=T)
colnames(IRall)<-c("number","source","target")
head(IRall)
centIRAll<-read.table("IR_op_CentralityRanking.tab", sep="\t", header=T, row.names = 1);
##################################################################
#Alt3
alt3all<-read.table("Alt3_op_edgelist.tab", sep="\t", header=T)
colnames(alt3all)<-c("number","source","target")
head(alt3all)
cent3AltAll<-read.table("Alt3_op_CentralityRanking.tab", sep="\t", header=T, row.names = 1);
##################################################################
#random30
random30<-read.table("random_edgelist.tab", sep="\t", header=T)
colnames(random30)<-c("number","source","target")
head(random30)
centRandomCR<-read.table("random_CentralityRanking.tab", sep="\t", header=T, row.names = 1);
#nodes:
#labchip
...
n7<-unique(c(as.character(alt5all$source),
             as.character(alt5all$target))); length(n7)#75
#IR all
n9<-unique(c(as.character(IRall$source),
             as.character(IRall$target))); length(n9)#46
#3alt all
n11<-unique(c(as.character(alt3all$source),
              as.character(alt3all$target))); length(n11)#126
#random30
n13<-unique(c(as.character(random30$source),
              as.character(random30$target))); length(n13)#158
...
#################################################################
getwd()
pdf("presence_ausence_nodes_AS_ex_alt5_ir_alt3_random_MD.pdf", width = 8.27,  height = 11.69)
ggplot(forPlotMelt, aes(x = variable, y = reorder(kds, -order), fill = factor(value)))  +
  ...
  theme(
    axis.text.x = element_text(angle = 90, hjust=0, vjust=-5,size=4),
